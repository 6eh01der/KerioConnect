#!/usr/bin/env pwsh
#https://github.com/6eh01der/KerioConnect/blob/main/Remove_emls_older_specific_date_for_specific_users.ps1
...
For ($i=0; $i -lt $LIST.Count; $i++) {
    Get-ChildItem "$PATH/$DOMAIN/$($LIST[$i])/*" -Exclude '#assoc', '__*', 'Journal' | Get-ChildItem -Include *.eml -Recurse | Where CreationTime -lt  $DATE | Remove-Item -Recurse
}
